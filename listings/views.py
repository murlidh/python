...
from .serializers import ListingSerializer, ListingDetailSerialzier
...
class ListingRetrieveView(RetrieveAPIView):
    queryset = Listing.objects.order_by('-list_date').filter(is_published=True)
    serializer_class = ListingDetailSerialzier
    lookup_field = 'slug'
...
#         for query in queryset:
#             now_days = (datetime.now(timezone.utc) - query.list_date).days

#             if days_passed != 0:
#                 if now_days > days_passed:
#                     slug = query.slug
#                     queryset = queryset.exclude(slug__iexact=slug)
